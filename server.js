...
function read(){return JSON.parse(fs.readFileSync(DATA,'utf8'))}function save(d){const s=new Date().toISOString().replace(/[:.]/g,'-');fs.copyFileSync(DATA,path.join(VERSIONS,s+'.json'));fs.writeFileSync(DATA,JSON.stringify(d,null,2),'utf8')}
...
فقط JSON معتبر برگردان؛ بدون Markdown و بدون توضیح اضافه.`;
  ...
  if(action==='article') return {instructions:rules,input:`متن موجود را از نظر ساختار، خوانایی، H2/H3، بولد هوشمند و لینک داخلی بهینه کن. اطلاعات جدیدِ تأییدنشده اضافه نکن. خروجی: {"body":"HTML...","excerpt":"..."}\n${JSON.stringify(payload)}`};
...
http.createServer(async(req,res)=>{const u=new URL(req.url,`http://${req.headers.host||'localhost'}`);try{
 ...
 if(u.pathname==='/api/admin/site'&&req.method==='PUT'){if(!auth(req))return j(res,401,{error:'unauthorized'});const x=JSON.parse(await body(req));save(x);return j(res,200,{ok:true})}
 ...
 if(u.pathname==='/api/ai'&&req.method==='POST'){if(!auth(req))return j(res,401,{error:'unauthorized'});const x=JSON.parse(await body(req)||'{}'),d=read();if(!['excerpt','article','generate','seo'].includes(x.action))return j(res,400,{error:'عملیات AI نامعتبر است'});try{const out=await callOpenAI(x.action,x.article||{},d,x.topic||'');return j(res,200,{ok:true,...out})}catch(e){console.error('AI error:',e);return j(res,502,{error:e.message||'خطا در سرویس هوش مصنوعی'})}}
